...
const HELP = {
  '/': {
    en: {
      ...
      features: [
        'The funnel opens every time the app does, for every role — admin, sales and partner alike. On a screen belonging to a sales rep or a partner it shows THEIR deals, not the whole unit: a line above it says how many more are in their business unit and are not theirs, so an empty funnel is never mistaken for an empty pipeline. It is a view filter and not a permission — everything stays counted in the budget, in sales by client and in the SAP reconciliation, which is what those figures are for. Switch to another view and it stays switched while you are working; open the app again tomorrow and you are back on the funnel. Two primary tabs (Summary, Details) plus secondary breakdown pills (Products, Reps, Clients, Funnel).',
    ...
    es: {
      ...
      features: [
        'El embudo se abre cada vez que se abre la aplicacion, para todos los roles — admin, comerciales y partners. En la pantalla de un comercial o de un partner ensena SUS oportunidades, no las de toda la unidad: una linea encima dice cuantas mas hay en su unidad que no son suyas, para que un embudo vacio nunca se confunda con un pipeline vacio. Es un filtro de vista y no un permiso — todo sigue contando en el presupuesto, en ventas por cliente y en la conciliacion con SAP. Si cambias de vista se queda cambiada mientras trabajas; manana al abrir vuelves al embudo. Dos pestanas principales (Resumen, Detalle) mas pills de desglose (Productos, Comerciales, Clientes, Embudo).',
    ...
    pt: {
      ...
      features: [
        'O funil abre sempre que a aplicacao abre, para todos os perfis — admin, comerciais e parceiros. No ecra de um comercial ou de um parceiro mostra os negocios DELE, e nao os da unidade toda: uma linha por cima diz quantos mais ha na unidade que nao sao dele, para um funil vazio nunca ser confundido com um pipeline vazio. E um filtro de vista e nao uma permissao — tudo continua a contar no orcamento, nas vendas por cliente e na reconciliacao com o SAP. Se mudares de vista, fica mudada enquanto estiveres a trabalhar; amanha ao abrir voltas ao funil. Duas abas principais (Resumo, Detalhe) mais pills de desdobramento (Produtos, Comerciais, Clientes, Funil).',
  ...
  '/deals': {
    en: {
      ...
      features: [
        ...
        'Opening a saved quote shows a moment of loading rather than the empty form it used to draw first: the defaults appeared and then snapped to what was actually quoted, and anybody reading quickly saw an empty quote for a deal that has one. For a partner it was worse — the catalogue starts empty, and an empty catalogue is what "you have no authorised products" is derived from, so every partner was briefly told they are not allowed to sell anything.',
        ...
        'A deal filed by a partner carries their name on the card, so it is recognisable in our pipeline before anybody opens it — and expanding it names who wrote it, which is a different question: a deal can belong to a partner because they filed it, or because one of us opened it on their behalf, and both carry the same company. That line shows only to an admin, who is the only role that can read anybody else\'s profile — the margin on it is theirs, and a discount on it is a concession to a company rather than to a customer. Inside, "Sold through" comes from the agreement with that partner rather than being asked again on every deal, which is what makes the end-customer price, our transfer price and the partner margin visible on the quote. There is nothing to configure: a distributor is a Full VAR — they sell, implement and carry first-line support — and anything we sell ourselves is direct. Three further roles once existed, inherited from the pricing spreadsheet and never used; they are gone from the choices, though a quote saved at one still prices at the rate it was saved at. A company can still state its own role in Permissions → Companies for an arrangement that differs. This applies to new quotes only: a deal already saved keeps exactly what it was quoted at, because opening a page to look at it must not change what it is worth. Where a saved deal says Direct and belongs to a partner, the screen says so and leaves the change to a person.',
        'The same client under two names: typing a client name instead of picking it from the list is how the table ended up with Hospital Ramon y Cajal twice, Telefonica twice, and 1.14 M of Catsalut split in two by a capital S. So while you type, the field checks what already exists and says so — three shapes of the same mistake: the identical name written differently (accents, capitals, a trailing S.A.), a name cut short by an import, and a one-letter typo. Tap a suggestion and the deal takes that client, linked to its account rather than merely spelled the same. It suggests and never corrects: two genuinely similar hospitals exist, and a name is not something the app should change behind you. It also stays quiet where a short name is the beginning of several real ones — "Remagna" starts seven different clinics, and guessing one of them would swap a duplicate for a deal filed against the wrong site.',
        ...
        'A discount banner at the top counts every state — pending, counter-offer, approved, rejected — and the badge beside it counts whichever of them is yours to move: a counter-offer for a distributor, a pending request for an approver. The "All" figure counts deals with any discount state at all rather than adding the buckets up, so the number and the list it filters to cannot disagree. Counter-offers used to be counted in no bucket, which made the banner say one while the list showed two.',
        ...
        'The certificate the client opens is written in their language, not in ours. It has no login and no profile \u2014 that browser has never been here \u2014 so the language cannot come from a stored preference the way it does everywhere else in the app: it comes from the country of the deal, which is the thing we know, because we sold it. A deal in Spain opens in Spanish, one in Portugal in Portuguese, one anywhere we have no rule for in English. Where the deal carries no country, the reader\u2019s own browser decides, which is a guess but theirs rather than ours. Before this, every certificate went out in English with the date written in Portuguese, because the date format was fixed in the code and the text was never translated at all.',
      ...
      mistakes: [
        ...
        'Ignoring the "this client may already exist" note and typing a new name anyway — the hospital then has two rows, and no report by client ever shows its total again.',
      ],
      seeAlso: [
    ...
    es: {
      ...
      features: [
        ...
        'Abrir una cotizacion guardada muestra un momento de carga en vez del formulario vacio que dibujaba antes: aparecian los valores por defecto y un instante despues saltaba a lo realmente cotizado, y quien leyera rapido veia una cotizacion vacia para una oportunidad que tiene una. Para un socio era peor — el catalogo empieza vacio, y de un catalogo vacio se deriva el mensaje "no tienes productos autorizados", asi que a cada socio se le decia brevemente que no puede vender nada.',
        ...
        'Una oportunidad presentada por un socio lleva su nombre en la tarjeta, y se reconoce en nuestro pipeline sin tener que abrirla — y al desplegarla se ve quien la escribio, que es otra pregunta: una oportunidad puede ser de un socio porque la creo el, o porque alguien de los nuestros la abrio por el, y ambas llevan la misma empresa. Esa linea solo la ve un admin, el unico rol que puede leer los perfiles de los demas — el margen es suyo, y un descuento es una concesion a una empresa y no a un cliente. Dentro, "Sold through" viene del acuerdo con ese socio en vez de preguntarse en cada oportunidad, que es lo que hace visibles el precio al cliente final, nuestro precio de transferencia y el margen del socio. No hay nada que configurar: un distribuidor es un Full VAR — vende, implementa y asume el primer nivel de soporte — y lo que vendemos nosotros es directo. Existieron otros tres roles, heredados de la hoja de precios y nunca usados; ya no se ofrecen, aunque una cotizacion guardada con uno sigue valorandose a la tarifa con que se guardo. Una empresa puede definir su propio rol en Permisos → Empresas si el acuerdo es distinto. Esto solo afecta a cotizaciones nuevas: una oportunidad ya guardada conserva exactamente aquello con lo que se cotizo, porque abrir una pagina para mirarla no puede cambiar lo que vale. Cuando una oportunidad guardada dice Directo y pertenece a un socio, la pantalla lo advierte y deja el cambio a una persona.',
        'El mismo cliente con dos nombres: escribir el nombre en vez de elegirlo de la lista es como la tabla acabo con el Hospital Ramon y Cajal dos veces, Telefonica dos veces y 1,14 M de Catsalut partidos en dos por una S mayuscula. Por eso, mientras escribes, el campo comprueba lo que ya existe y lo dice — tres formas del mismo error: el nombre identico escrito de otra manera (acentos, mayusculas, un S.A. al final), un nombre cortado por una importacion, y una errata de una letra. Pulsa una sugerencia y la oportunidad toma ese cliente, enlazado a su cuenta y no solo escrito igual. Sugiere y nunca corrige: hay hospitales realmente parecidos, y un nombre no es algo que la aplicacion deba cambiar por su cuenta. Tambien se calla cuando un nombre corto es el principio de varios reales — "Remagna" empieza siete clinicas distintas, y adivinar una cambiaria un duplicado por una oportunidad archivada en el centro equivocado.',
        ...
        'Un banner de descuentos en la parte superior cuenta los cuatro estados — pendiente, contraoferta, aprobado, rechazado — y el distintivo junto a el cuenta el que te toca mover a ti: una contraoferta si eres distribuidor, una solicitud pendiente si apruebas. La cifra de "Todos" cuenta oportunidades con cualquier estado de descuento en vez de sumar los grupos, asi que el numero y la lista que filtra no pueden discrepar. Las contraofertas no se contaban en ningun grupo, y por eso el banner decia una y la lista mostraba dos.',
        ...
        'El certificado que abre el cliente va escrito en su idioma, no en el nuestro. No tiene login ni perfil \u2014 ese navegador nunca ha estado aqui \u2014 asi que el idioma no puede venir de una preferencia guardada como en el resto de la aplicacion: viene del pais del deal, que es lo que si sabemos, porque lo vendimos nosotros. Un deal en Espana abre en espanol, uno en Portugal en portugues, y uno de un pais para el que no hay regla en ingles. Si el deal no lleva pais, decide el navegador de quien lee, que es una suposicion pero suya y no nuestra. Antes de esto todos los certificados salian en ingles con la fecha escrita en portugues, porque el formato estaba fijo en el codigo y el texto no estaba traducido en absoluto.',
      ...
      mistakes: [
        ...
        'Ignorar el aviso de "este cliente quiza ya existe" y escribir el nombre nuevo igualmente — el hospital queda en dos filas, y ningun informe por cliente vuelve a ensenar su total.',
      ],
      seeAlso: [
    ...
    pt: {
      ...
      features: [
        ...
        'Abrir uma proposta guardada mostra um momento de carregamento em vez do formulario vazio que desenhava antes: apareciam os valores por omissao e um instante depois saltava para o que estava realmente proposto, e quem lesse depressa via uma proposta vazia para um negocio que tem uma. Para um parceiro era pior — o catalogo comeca vazio, e e de um catalogo vazio que se deriva a mensagem "nao tens produtos autorizados", por isso a cada parceiro era dito por instantes que nao pode vender nada.',
        ...
        'Um negocio criado por um parceiro leva o nome dele no cartao, e reconhece-se no nosso pipeline sem ser preciso abri-lo — e ao expandi-lo ve-se quem o escreveu, que e outra pergunta: um negocio pode ser de um parceiro porque foi ele que o criou, ou porque alguem nosso o abriu por ele, e os dois levam a mesma empresa. Essa linha so aparece ao admin, o unico perfil que pode ler os perfis dos outros — a margem e dele, e um desconto ali e uma concessao a uma empresa e nao a um cliente. La dentro, o "Sold through" vem do acordo com esse parceiro em vez de ser perguntado em cada negocio, e e isso que torna visiveis o preco ao cliente final, o nosso preco de transferencia e a margem do parceiro. Nao ha nada para configurar: um distribuidor e um Full VAR — vende, implementa e assume o primeiro nivel de suporte — e o que vendemos nos e directo. Existiram mais tres papeis, herdados da folha de precos e nunca usados; deixaram de ser oferecidos, embora uma proposta guardada com um deles continue a valer a taxa a que foi guardada. Uma empresa pode na mesma definir o seu proprio papel em Permissoes → Empresas se o acordo for diferente. Isto aplica-se so a propostas novas: um negocio ja guardado mantem exactamente aquilo a que foi cotado, porque abrir uma pagina para a ver nao pode mudar o que ela vale. Onde um negocio guardado diz Directo e pertence a um parceiro, o ecra di-lo e deixa a mudanca a uma pessoa.',
        'O mesmo cliente com dois nomes: escrever o nome em vez de o escolher da lista e como a tabela ficou com o Hospital Ramon y Cajal duas vezes, a Telefonica duas vezes e 1,14 M do Catsalut partidos em dois por um S maiusculo. Por isso, enquanto escreves, o campo verifica o que ja existe e di-lo — tres formas do mesmo erro: o nome identico escrito de outra maneira (acentos, maiusculas, um S.A. no fim), um nome cortado por uma importacao, e uma gralha de uma letra. Toca numa sugestao e o negocio passa a ter esse cliente, ligado a conta dele e nao apenas escrito da mesma maneira. Sugere e nunca corrige: ha hospitais genuinamente parecidos, e um nome nao e coisa que a aplicacao deva mudar sem o utilizador. Tambem se cala quando um nome curto e o principio de varios reais — "Remagna" comeca sete clinicas diferentes, e adivinhar uma delas trocaria um duplicado por um negocio arquivado no sitio errado.',
        ...
        'Um banner de descontos no topo conta os quatro estados — pendente, contraproposta, aprovado, recusado — e o sinal ao lado conta aquele que e a tua jogada: uma contraproposta se es distribuidor, um pedido pendente se aprovas. O numero de "Todos" conta negocios com qualquer estado de desconto em vez de somar os grupos, por isso o numero e a lista que ele filtra nao podem discordar. As contrapropostas nao eram contadas em grupo nenhum, e por isso o banner dizia um e a lista mostrava dois.',
        ...
        'O certificado que o cliente abre vai escrito na lingua dele e nao na nossa. Nao tem login nem perfil \u2014 aquele browser nunca ca entrou \u2014 por isso a lingua nao pode vir de uma preferencia guardada como no resto da aplicacao: vem do pais do negocio, que e a coisa que nos sabemos, porque fomos nos que o vendemos. Um negocio em Espanha abre em espanhol, um em Portugal em portugues, e um de um pais para o qual nao ha regra em ingles. Se o negocio nao trouxer pais, decide o browser de quem esta a ler, que e um palpite mas dele e nao nosso. Antes disto todos os certificados saiam em ingles com a data escrita em portugues, porque o formato estava cravado no codigo e o texto nao estava traduzido de todo.',
      ...
      mistakes: [
        ...
        'Ignorar o aviso de "este cliente talvez ja exista" e escrever o nome novo na mesma — o hospital fica em duas linhas, e nenhum relatorio por cliente volta a mostrar o total dele.',
      ],
      seeAlso: [
  ...
  '/clients': {
    en: {
      ...
      features: [
        ...
        'Putting a client into a group: the client form has a Group field \u2014 pick the parent and the client moves under it, with its money counted in the group total from the next reload. It was only on the Accounts page before, which is a different screen, and knowing which of the two pages does what is why nobody used the hierarchy that the database has had all along. A client cannot be its own parent, nor the parent of anything already beneath it: that is how you build a group that contains itself, and the total then never finishes adding up. Leave the field empty for a client that belongs to no group, which is most of them.',
    ...
    es: {
      ...
      features: [
        ...
        'Meter un cliente en un grupo: el formulario de cliente tiene el campo Grupo \u2014 elige el padre y el cliente pasa a colgar de el, con su dinero sumado al total del grupo en la siguiente carga. Antes solo estaba en la pagina de Cuentas, que es otra pantalla, y tener que saber cual de las dos hace que es la razon por la que nadie uso una jerarquia que la base de datos siempre tuvo. Un cliente no puede ser padre de si mismo ni de quien ya cuelga de el: asi se construye un grupo que se contiene, y el total no termina nunca de sumar. Dejalo vacio para un cliente que no pertenece a ningun grupo, que son la mayoria.',
    ...
    pt: {
      ...
      features: [
        ...
        'Meter um cliente num grupo: o formulario de cliente tem o campo Grupo \u2014 escolhe o pai e o cliente passa para baixo dele, com o dinheiro somado no total do grupo no carregamento seguinte. Antes so existia na pagina de Contas, que e outro ecra, e ter de saber qual das duas faz o que e a razao pela qual ninguem usou uma hierarquia que a base de dados sempre teve. Um cliente nao pode ser pai de si proprio nem de quem ja esta por baixo dele: e assim que se faz um grupo que se contem, e o total passa a nunca acabar de somar. Deixa vazio para um cliente que nao pertence a grupo nenhum, que sao a maioria.',
  ...
  '/verification': {
    en: {
      title: 'Verification — SAP vs BusinessBook',
      description: 'The same month in both books, and where they differ. SAP is what was invoiced; BusinessBook is what the team recorded.',
      features: [
        'Pick a company — VGT (1715), ECT (1766) or Iberia for the two together — and a month.',
        'The header gives both totals and the gap between them, split into exactly three causes that add back up to it: invoiced and not recorded, recorded and not invoiced, and same client with a different figure.',
        'Only in SAP is the work: revenue that reached the customer and never reached a deal.',
        'Only in BusinessBook is usually the right deal in the wrong month, not a mistake.',
        'In both lists every matched client with the difference, biggest first.',
      ],
      steps: [
        '1. Choose the company and the month.',
        '2. Import SAP month, and paste the export straight out of Excel: CustomerName, Net Sales, Gross Margin, Gross Margin %.',
        '3. Check the preview — how many rows it read, the totals, and anything it could not read.',
        '4. Replace the month. Importing the same month twice leaves one month, not two.',
        '5. Work down Only in SAP. Where a customer is the same one under another name, use link and pick it from the BusinessBook side; the pairing is remembered.',
      ],
      shortcuts: [],
      mistakes: [
        'Reading the gap as an error. Most of it is timing — a deal recognised in August that SAP billed in September shows on both sides in different months.',
        'Importing without choosing the company first: a month belongs to VGT or to ECT, never to both.',
        'Trusting the percentage column. The percentage is derived from the two figures; where the file disagrees with itself, the figures are used and the row is flagged.',
      ],
      seeAlso: ['/', '/deals', '/budget'],
      admin: 'Importing a month is restricted to admin and manager — it is the official book.',
      distributor: 'Not available: this page compares our own revenue.',
      viewer: 'Not available.',
    },
    es: {
      title: 'Verificación — SAP vs BusinessBook',
      description: 'El mismo mes en los dos libros, y donde difieren. SAP es lo facturado; BusinessBook es lo que registro el equipo.',
      features: [
        'Elige empresa — VGT (1715), ECT (1766) o Iberia para las dos juntas — y un mes.',
        'La cabecera da los dos totales y la diferencia, repartida en exactamente tres causas que suman esa diferencia: facturado y no registrado, registrado y no facturado, y mismo cliente con cifra distinta.',
        'Solo en SAP es el trabajo: ingresos que llegaron al cliente y nunca llegaron a una oportunidad.',
        'Solo en BusinessBook suele ser la oportunidad correcta en el mes equivocado, no un error.',
        'En ambos lista cada cliente emparejado con su diferencia, de mayor a menor.',
      ],
      steps: [
        '1. Elige la empresa y el mes.',
        '2. Importar mes de SAP y pega la exportacion tal cual de Excel: CustomerName, Net Sales, Gross Margin, Gross Margin %.',
        '3. Revisa la vista previa — cuantas filas leyo, los totales y lo que no pudo leer.',
        '4. Reemplaza el mes. Importar dos veces el mismo mes deja un mes, no dos.',
        '5. Baja por Solo en SAP. Si un cliente es el mismo con otro nombre, usa vincular y eligelo del lado de BusinessBook; el emparejamiento queda guardado.',
      ],
      shortcuts: [],
      mistakes: [
        'Leer la diferencia como un error. La mayor parte es calendario — una oportunidad reconocida en agosto que SAP facturo en septiembre aparece en los dos lados en meses distintos.',
        'Importar sin elegir empresa: un mes pertenece a VGT o a ECT, nunca a las dos.',
        'Fiarse de la columna de porcentaje. El porcentaje se deriva de las dos cifras; si el fichero se contradice, se usan las cifras y la fila se marca.',
      ],
      seeAlso: ['/', '/deals', '/budget'],
      admin: 'Importar un mes esta restringido a admin y manager — es el libro oficial.',
      distributor: 'No disponible: esta pagina compara ingresos nuestros.',
      viewer: 'No disponible.',
    },
    pt: {
      title: 'Verificacao — SAP vs BusinessBook',
      description: 'O mesmo mes nos dois livros, e onde e que diferem. O SAP e o que foi faturado; o BusinessBook e o que a equipa registou.',
      features: [
        'Escolhe a empresa — VGT (1715), ECT (1766) ou Iberia para as duas juntas — e um mes.',
        'O cabecalho da os dois totais e a diferenca entre eles, repartida em exatamente tres causas que voltam a somar essa diferenca: faturado e nao registado, registado e nao faturado, e mesmo cliente com valor diferente.',
        'So no SAP e o trabalho: receita que chegou ao cliente e nunca chegou a um negocio.',
        'So no BusinessBook e quase sempre o negocio certo no mes errado, e nao um erro.',
        'Nos dois lista cada cliente emparelhado com a sua diferenca, do maior para o menor.',
      ],
      steps: [
        '1. Escolhe a empresa e o mes.',
        '2. Importar mes do SAP e cola a exportacao tal como sai do Excel: CustomerName, Net Sales, Gross Margin, Gross Margin %.',
        '3. Confere a pre-visualizacao — quantas linhas leu, os totais, e o que nao conseguiu ler.',
        '4. Substitui o mes. Importar duas vezes o mesmo mes deixa um mes, nao dois.',
        '5. Desce pela lista So no SAP. Quando um cliente e o mesmo com outro nome, usa ligar e escolhe-o do lado do BusinessBook; a ligacao fica guardada.',
      ],
      shortcuts: [],
      mistakes: [
        'Ler a diferenca como erro. A maior parte e calendario — um negocio reconhecido em agosto que o SAP faturou em setembro aparece dos dois lados em meses diferentes.',
        'Importar sem escolher a empresa: um mes pertence a VGT ou a ECT, nunca as duas.',
        'Confiar na coluna da percentagem. A percentagem e derivada dos dois valores; quando o ficheiro se contradiz, usam-se os valores e a linha fica assinalada.',
      ],
      seeAlso: ['/', '/deals', '/budget'],
      admin: 'Importar um mes esta restrito a admin e manager — e o livro oficial.',
      distributor: 'Nao disponivel: esta pagina compara receita nossa.',
      viewer: 'Nao disponivel.',
    },
  },
  '/approvals': {
    en: {
