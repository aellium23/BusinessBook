...
function PctBadge({ value, reference, label, invert = false }) {
...
function BUPerformanceCard({ bu, color, label, actMTD, actYTD, planMTD, planYTD, pyMTD, pyYTD, cycle, mtdLabel, ytdLabel }) {
  const planMTDpct = pct(actMTD, planMTD)
  const planYTDpct = pct(actYTD, planYTD)
  const pyMTDpct   = pct(actMTD, pyMTD)
  const pyYTDpct   = pct(actYTD, pyYTD)
    ...
    return (
      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden relative">
        <div className="h-full rounded-full transition-all"
          style={{ width:`${Math.min(p,100)}%`, background: over ? '#1D9E75' : c }}/>
        {over && <div className="absolute right-0 top-0 h-full w-1 rounded-full bg-green-500"/>}
      </div>
    )
  }
    ...
    <div className="bg-white rounded-xl border-2 shadow-sm overflow-hidden" style={{ borderColor: color }}>
      ...
      <div className="px-4 pt-3 pb-2 flex items-center justify-between"
        ...
        <div>
          ...
          <p className="text-[10px] text-gray-400">{cycle} cycle · Apr 2026</p>
        </div>
      ...
      <div className="px-4 py-3 border-b border-gray-50">
        <div className="flex items-start justify-between mb-1.5">
          ...
          <div className="text-right space-y-0.5 mt-1">
            ...
            <div/>
            <PctBadge value={actMTD} reference={pyMTD} label="PY"/>
          </div>
        </div>
        <Bar value={actMTD} max={planMTD} c={color}/>
        <div className="flex justify-between text-[9px] text-gray-400 mt-0.5">
          <span>Plan: {formatK(planMTD*1000)}</span>
          <span>PY: {formatK(pyMTD*1000)}</span>
        </div>
      </div>
      ...
      <div className="px-4 py-3">
        <div className="flex items-start justify-between mb-1.5">
          ...
          <div className="text-right space-y-0.5 mt-1">
            ...
            <div/>
            <PctBadge value={actYTD} reference={pyYTD} label="PY"/>
          </div>
        </div>
        <Bar value={actYTD} max={planYTD} c={color}/>
        <div className="flex justify-between text-[9px] text-gray-400 mt-0.5">
          <span>Plan: {formatK(planYTD*1000)}</span>
          <span>PY: {formatK(pyYTD*1000)}</span>
        </div>
      </div>
    </div>
  )
...
function PerformanceSection({ agg, budgetTotals, fy25, deals, budget, activeCycle, isAdmin }) {
  const fyIdx    = getFYMonthIndex()
  const curMonth = MONTHS_K[fyIdx]
  const ytdMonths = MONTHS_K.slice(0, fyIdx + 1)

  // Actuals from deals (Invoiced)
  function getActuals(bu) {
    return deals.filter(d => d.bu === bu && d.stage === 'Invoiced' && !d.is_intercompany_mirror)
  }

  function sumMonth(dealList, month) {
    return dealList.reduce((s, d) => s + (d[month]||0), 0) / 1000
  }
  function sumYTD(dealList) {
    return ytdMonths.reduce((s, m) => s + sumMonth(dealList, m), 0)
  }

  // Plan from budget
  function getPlanMonth(bu, month) {
    return ['ns_int','ns_ext'].reduce((s, key) => {
      const row = budget.find(r => r.bu===bu && r.cycle===activeCycle && r.pl_key===key)
      return s + (row?.[month]||0)
    }, 0)
  }
  function getPlanYTD(bu) {
    return ytdMonths.reduce((s, m) => s + getPlanMonth(bu, m), 0)
  }

  // PY from fy25_actuals (homologous period)
  function getPYMonth(bu, month) {
    const row = fy25.find(r => r.bu===bu && r.pl_key==='ns')
    return row ? (row[month]||0) : 0
  }
  function getPYYTD(bu) {
    return ytdMonths.reduce((s, m) => s + getPYMonth(bu, m), 0)
  }

  const vgtDeals = getActuals('VGT')
  const ectDeals = getActuals('ECT')
  const mtdLabel = MONTHS_LABEL[fyIdx]
  const ytdLabel = fyIdx === 0 ? MONTHS_LABEL[0] : `${MONTHS_LABEL[0]}–${MONTHS_LABEL[fyIdx]}`

  const cards = [
    {
      bu:'VGT', color:'#1D9E75', label:'VGT · Portugal',
      actMTD:  sumMonth(vgtDeals, curMonth),
      actYTD:  sumYTD(vgtDeals),
      planMTD: getPlanMonth('VGT', curMonth),
      planYTD: getPlanYTD('VGT'),
      pyMTD:   getPYMonth('VGT', curMonth),
      pyYTD:   getPYYTD('VGT'),
    },
    {
      bu:'ECT', color:'#D85A30', label:'ECT · Spain',
      actMTD:  sumMonth(ectDeals, curMonth),
      actYTD:  sumYTD(ectDeals),
      planMTD: getPlanMonth('ECT', curMonth),
      planYTD: getPlanYTD('ECT'),
      pyMTD:   getPYMonth('ECT', curMonth),
      pyYTD:   getPYYTD('ECT'),
    },
  ]
  ...
  // Iberia consolidated
  const iberiaCard = {
    bu:'ALL', color:'#0D2137', label:'Iberia · Consolidated',
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">
          Sales performance · {activeCycle} · MTD & YTD
        </p>
        <span className="text-[10px] text-gray-400">
          Current month: <strong>{mtdLabel}</strong> · YTD: <strong>{ytdLabel}</strong>
        </span>
      </div>

      {/* VGT + ECT side by side */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {cards.map(c => (
          <BUPerformanceCard key={c.bu} {...c} cycle={activeCycle} mtdLabel={mtdLabel} ytdLabel={ytdLabel}/>
        ))}
      </div>

      {/* Iberia consolidated - only for admin */}
      {isAdmin && (
        <BUPerformanceCard {...iberiaCard} cycle={activeCycle} mtdLabel={mtdLabel} ytdLabel={ytdLabel}/>
      )}
    </div>
  )
}
