...
function Trend({ value, reference }) {
  ...
  if (pct > 0) return (
    <span className="flex items-center gap-0.5 text-green-600 text-micro font-medium">
      <TrendingUp size={9}/>{pct.toFixed(1)}%
    </span>
  )
  return (
    <span className="flex items-center gap-0.5 text-red-500 text-micro font-medium">
                      ...
                      return (
                        <tr key={plKey} className={`border-b ${isTotal ? 'bg-navy/[0.06] border-t-2 border-navy/20 font-bold' : ''}`}>
                          ...
                          {MONTHS_K.map((m, mi) => {
                            ...
                            return (
                              <td key={m} className="px-0.5 sm:px-1 py-1 text-center">
                                <p className="text-micro sm:text-xs font-semibold text-gray-800">{left ? left.toFixed(1) : '—'}</p>
                                <p className="text-micro text-gray-400">{right ? right.toFixed(1) : '—'}</p>
                                {(left || right) ? (
                                  <p className={`text-micro font-bold ${varColor(variance, isCost)}`}>
                          ...
                          <td className="px-1 sm:px-2 py-1 text-center border-l-2 border-gray-200">
                            <p className="text-micro sm:text-xs font-bold text-gray-800">{leftYTD.toFixed(1)}</p>
                            <p className="text-micro text-gray-400">{rightYTD.toFixed(1)}</p>
                            {(leftYTD || rightYTD) ? (
                              <p className={`text-micro font-bold ${varColor(leftYTD - rightYTD, isCost)}`}>
          ...
          return (
            <button key={cycle} onClick={() => setActiveCycle(cycle)}
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {isActive && <span className="text-micro bg-white/70 px-1.5 py-0.5 rounded font-medium" style={{ color: cfg.color }}>{tr("budget_active")}</span>}
              </div>
                ...
                return (
                  <tr key={key}
                        ...
                        <td key={mk} className="p-0.5">
                          {input && activeBu !== 'ALL' ? (
                            <input
                              ...
                              placeholder="—"
                              className={`w-full text-center text-micro sm:text-xs px-0.5 sm:px-1 py-1.5 rounded-lg transition-all outline-none
                                ${isFocused
                            ...
                            <div className={`text-center px-0.5 sm:px-1 py-1.5 text-micro sm:text-xs font-bold ${
                    ...
                    <td className="px-1 sm:px-3 py-2 text-center">
                      <span className={`text-micro sm:text-xs font-bold ${
                        isTotal
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-4">
        ...
        {fctForm && (
          <div className="border border-blue-200 rounded-xl p-3 space-y-3 bg-blue-50/30">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-micro text-blue-500">{tr("budget_prefilled")}</span>
            </div>

            {['ns_ext', 'ns_int'].map(plKey => (
              <div key={plKey}>
                <p className="text-micro font-semibold text-gray-500 uppercase mb-1">
                  {plKey === 'ns_ext' ? tr("budget_ext_sales") : tr("budget_int_sales")}
                </p>
                <div className="grid grid-cols-4 sm:grid-cols-6 gap-1">
                  {MONTHS_K.map((m, i) => (
                    <div key={m}>
                      <label className="text-micro text-gray-400">{MONTHS[i]}</label>
                ...
                <p className="text-micro text-right text-gray-500 mt-0.5">
            ...
            <div>
              <label className="text-micro text-gray-500">{tr("budget_notes_opt")}</label>
        ...
        {fctSnapshots.length > 0 && (
          <div className="space-y-2">
            <p className="text-micro font-semibold text-gray-400 uppercase">{tr("budget_snapshot_history")}</p>
            {(() => {
              const byDate = {}
                  ...
                  <div key={i} className="bg-gray-50 rounded-lg overflow-hidden">
                    <button onClick={() => {
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`text-micro font-bold px-1.5 py-0.5 rounded ${CYCLE_CONFIG[group.cycle]?.badge}`}>
                          {group.cycle}
                        </span>
                        <span className="text-micro text-gray-500">{group.bu}</span>
                      ...
                      <div className="flex items-center gap-2">
                        {group.notes && <span className="text-micro text-gray-400 max-w-32 truncate">{group.notes}</span>}
                          ...
                          <div className="grid grid-cols-6 gap-0.5">
                            {MONTHS.map((m, mi) => (
                              <div key={m} className="text-center">
                                ...
                                <p className="text-micro font-semibold text-gray-700">{Number(extRow[MONTHS_K[mi]]) || '—'}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-6 gap-0.5">
                            {MONTHS.map((m, mi) => (
                              <div key={m} className="text-center">
                                <p className="text-micro text-gray-300">{m}</p>
                                <p className="text-micro font-semibold text-gray-700">{Number(intRow[MONTHS_K[mi]]) || '—'}</p>
                              </div>
                            ))}
                          </div>
                        </div>
